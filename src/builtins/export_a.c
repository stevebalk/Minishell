...
/*   Updated: 2023/11/20 16:49:38 by jopeters         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void export_arg(t_list **env_llst, t_list **env_llst_sorted, char *str, int str_offset)
{
	...
	tmp_string_no_quotes = get_string_without_symbols(str + str_offset, '"');
	printf(" --> clean var name >%s<\n", clean_var_no_equal);
		// Check if variable is there if not add variable
	if (find_var_in_llst(env_llst, clean_var_no_equal))
	{
		update_content_in_node(find_var_in_llst(env_llst, clean_var_no_equal), tmp_string_no_quotes);
		update_content_in_node(find_var_in_llst(env_llst_sorted, clean_var_no_equal), str + str_offset);
	}
	else
	{
		add_variable_to_llst(env_llst, tmp_string_no_quotes);
		add_variable_to_llst(env_llst_sorted, str + str_offset);
	}
